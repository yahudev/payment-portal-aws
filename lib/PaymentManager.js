...
module.exports = function(PaymentOrder, redis) {
  ...
  const checkOrder = async (customerName, ref) => {
    ...
    order = await PaymentOrder.findOne({customerName, ref});
    if (!order) {
      return null;
    }
  ...
  const startProcess = async (order, vendor, key) => {
    console.info('startProcess: hashed ', shortHash(key), ' key ', key);
    order.process = {
      vendor,
      key: shortHash(key),
    };
  ...
  const findByProcess = async (key) => {
    console.info('findByProcess: hashed ', shortHash(key),' key ', key);
