...
module.exports = function Paypal(config, app, PaymentOrder) {
  ...
  const initPaymentProcess = async (paymentOrder, processKey) => {
  ...
  const saveProcessFailure = async (processKey) => {
  ...
  const saveProcessSuccess = async (processKey) => {
    await PaymentOrder.findOneAndUpdate({
      'process.processKey': processKey,
    }, {
      'process.isComplete': true,
    ...
    try {
      ...
      paypal.payment.create(payReq, async (error, payment) => {
        ...
        if (links.hasOwnProperty('approval_url')) {
          ...
          let processKey = new URL(approvalUrl).query.token;
          await initPaymentProcess(paymentOrder, processKey);
